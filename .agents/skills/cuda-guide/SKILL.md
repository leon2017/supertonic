---
name: cuda-guide
description: "[DEPRECATED] This skill is no longer used."
version: 0.0.0
---

# DEPRECATED

This skill was for a video stabilization project and is no longer relevant.
...
```bash
# Compile CUDA code
nvcc -arch=sm_80 -O3 -o program main.cu
nvcc -arch=native -lineinfo -o program main.cu

# CMake build
cmake -B build -DCMAKE_CUDA_ARCHITECTURES="70;80;86;90"
cmake --build build -j$(nproc)

# Runtime debugging
compute-sanitizer ./program                      # Memory errors
compute-sanitizer --tool racecheck ./program     # Shared memory races
compute-sanitizer --tool initcheck ./program     # Uninitialized reads
